/**
 * PerspectiveLens Analysis Panel Controller V2
 * Refactored to use modular stage renderers
 * Progressive rendering support for Gemini Nano multi-stage analysis
 */

// Import PanelRenderer (will be loaded as ES module)
...
(function() {
  ...
  if (window.PerspectiveLensPanel) {
    console.log('[Panel] Already loaded, skipping...');
    return;
  }
  ...
  class AnalysisPanelV2 {
    ...
    attachEventListeners() {
      ...
      // Listen for perspective modal trigger
      document.addEventListener('click', (e) => {
        if (e.target.id === 'pl-view-perspectives') {
          e.preventDefault();
    ...
    showAnalysisProgressive(stage, stageData, perspectives) {
      console.log(`[Panel] Showing stage ${stage} progressively`, {
    ...
    showAnalysis(analysis) {
      console.log('[Panel] Showing complete analysis', {
        hasStory: !!analysis.story_summary,
        hasConsensus: !!analysis.consensus,
        hasDisputes: !!analysis.factual_disputes,
        hasPerspectives: !!analysis.perspective_differences,
        perspectivesCount: analysis.perspectives?.length || 0
      });
  ...
  window.PerspectiveLensPanel = new AnalysisPanelV2();
  console.log('[Panel] Global instance created');
})();
